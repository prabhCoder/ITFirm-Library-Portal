...
public class IssueReport {
	...
	File exactFile;
	{
		...
		firstSheet = workbook.createSheet("ISSUE REPORT");
	...
	public void createEX(OutputStream out) {
		...
		ResultSet rs = book.displaybooks_history(getISBN());
		...
		try {
			...
			IssueReport cls = new IssueReport(recordToAdd);
			cls.createFile(out);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	IssueReport(List<List> l1) throws Exception {
		try {
			for (int j = 0; j < l1.size(); j++) {
				...
				for (int k = 0; k < l2.size(); k++) {
					...
					hsfstyle.setBottomBorderColor(IndexedColors.BLACK
							.getIndex());
